function geoDist = GeodesicSaliency3(adjcMatrix, bg_all, BGindex, colDistM, posDistM)
...
bdIds = unique(union(bg_all,BGindex));
bondIds = BoundaryAnalysis(colDistM, posDistM, bg_all);
% Pbd = Pbg(bg_all);
% [~,ind] = sort(Pbd,'descend');% ½µÐò;
% bg_num = ceil(0.8*size(bg_all,1));
% bondIds = ind(1:bg_num);
bgIds = unique(union(bondIds,BGindex));
