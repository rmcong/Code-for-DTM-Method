function [sup_feat,color_weight] = extractSupfeat(input_im,input_imlab,regions,row,col,sup_num)
    ...
    for r = 1:sup_num %% check the superpixel not along the image sides
        ...
        %sup_feat =[mean(R(ind)),mean(G(ind)),mean(B1(ind)),mean(L(ind)),mean(A(ind)),mean(B2(ind)),mean(indxy(:,2))/col,mean(indxy(:,1))/row];%原始
        sup_feat(r,:) = [mean(R(ind)),mean(G(ind)),mean(B1(ind)),mean(L(ind)),mean(A(ind)),mean(B2(ind)),mean(indxy(:,2))/col,mean(indxy(:,1))/row];%丛改150821
    end
