...
if( ~isfield(pChns,'complete') || pChns.complete~=1 || isempty(I) )
    %%%%%%%%%%%%%%%%%%%%%%%
%   p=struct('enabled',1,'name','saliencycolor','hFunc',@normalizedRGB,'padWith',0);%for user interface
  ...
  pChns.pGradHist = getPrmDflt( pChns.pGradHist, {'enabled',1,...
    'binSize',[],'nOrients',6,'softBin',-1,'useHog',2,'clipHog',.2}, 1 );
...
OrI=I;
if size(OrI,3)<=1, OrI=repmat(OrI,[1 1 3]); end
...
%       C=single(feval(p(i).hFunc,OrI,p(i).pFunc{:}));
%       chns=addChn(chns,C,p(i).name,p(i),p(i).padWith,h,w);
%     end
%     %modulate
%     chns.data{1}=chns.data{1}.*repmat(chns.data{4},[1,1,chns.info(1).nChns]);
%     chns.data{2}=chns.data{2}.*repmat(chns.data{4},[1,1,chns.info(2).nChns]);
%     chns.data{3}=chns.data{3}.*repmat(chns.data{4},[1,1,chns.info(3).nChns]);
% end
p=pChns.pCustom;
for i=find( [p.enabled] )
